...
@Path("authentication")
//creeer le jeton avec cette classe
public class AuthenticationEndPoint {
    ...
    json donc il faut envoier tous lobject
     public Response authenticateUser(Credentials cred) {
        ...
        sinon on travail avec le formParam wnbadlo:@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    */
    ...
    public Response authenticateUser(@FormParam("username") String username, @FormParam("password") String password) {
        ...
        try {
            ...
            return Response.status(Response.Status.FORBIDDEN).build();//ou anothorized
        }
    }
    ...
    private String issueToken(String username) {
        ...
        String jwtToken = Jwts.builder()
                .setSubject(username)  //le fournisseur fix ces information
                .setIssuer(uriInfo.getAbsolutePath().toString()) //qui a generer le jeton
                .setIssuedAt(new Date())
                .setExpiration(toDate(LocalDateTime.now().plusMinutes(15L))) //15 min
                .signWith(SignatureAlgorithm.HS512, key)
                .compact();//pour compacter les donnes json en string
